...
class Map(object):
    def __init__(self, start_data, data):
        ...
        self.replay_url = REPLAY_URLS["na"] + start_data[
            'replay_id']  # String Replay URL          # TODO: Use Client Region
        ...
        self.tiles = [[] for _ in range(12)]  # List of 8 (+ extra) Players Tiles
        self.cities = []  # List of City Tiles
        self.generals = [None for _ in range(12)]  # List of 8 (+ extra) Generals (None if not found)
        self._set_generals()
    ...
    def find_city(self, of_type=None, not_of_type=None, not_in_path=None, find_largest=True,
                  include_general=False):
        """

        :param of_type: Integer
        :param not_of_type: Integer
        :param not_in_path: [Tile
        :param find_largest: Boolean
        :param include_general:
        :return:
        """
    ...
    def find_largest_tile(self, of_type=None, not_in_path=None,
        ...
        :param of_type: Integer
        :param not_in_path: [Tile]
        :param include_general: False|True|Int Acceptable Largest|0.1->0.9 Ratio
        :return:
        """
        if not_in_path is None:
            not_in_path = []
        ...
        largest = None
        for tile in self.tiles[of_type]:  # Check each of_type tile
            if largest is None or largest.army < tile.army:  # New largest
                if not tile.isGeneral and tile not in not_in_path:  # Exclude general and path
                    largest = tile

        if include_general > 0 and general is not None and general not in not_in_path:  # Handle include_general
            ...
            if largest is None:
                largest = general
            elif include_general is True and largest.army < general.army:
                largest = general
